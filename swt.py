...
def apply_swt(edges: Image, gradients: Gradients, dark_on_bright: bool = True) -> Image:
    """
...
def minimum_area_bounding_box(points: np.ndarray) -> np.ndarray:
    ...
    for i in range(len(hull.vertices) - 1):
        ...
        # print(a, b)
    return points
def discard_non_text(swt: Image, components: List[Component]) -> Tuple[Image, List[Component]]:
    """
...
def filter( img:Image, components: List[Component],ration=4,max=1000,min=30) -> Tuple[Image, List[Component]]:
    valid_components = []  # type: List[Component]
    labels = np.zeros_like(img).astype(np.uint8)
    for component in components:
        compon= np.array([[p.y, p.x] for p in component])
...
def main():
    im = cv2.imread('merge.png', cv2.IMREAD_COLOR)
    im = get_grayscale(im)
    edges = get_edges(im)
    gradients = get_gradients(im)
    grad_direction=get_gradient_directions(gradients)
    swt = apply_swt(edges, gradients, True)
    ...
    a,b=discard_non_text(swt,  components)
    cv2.imshow('Image', im)
    cv2.imshow('Edges', edges)
    cv2.imshow('gdire', grad_direction)
    cv2.imshow('SWT  ', swt)
